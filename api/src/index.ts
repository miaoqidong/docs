...
export interface RawSubscription {
   ...
   *
   * 负数 id 被 趣助理 内部使用, 用户无法添加负数 id 的订阅
   *
   */
  id: Integer;
  ...
  /**
   * 趣助理 会定时或者用户手动刷新请求这个链接, 如果返回的订阅的 version 大于应用订阅当前的 version , 则更新
   *
   ...
   * ```
   * 
   * 支持相对地址, 如果 {@link updateUrl} 或者用户添加订阅时填写的链接是 `https://趣助理.li/趣助理.json5` 并且 {@link checkUpdateUrl} 是 `./趣助理.version.json5`
   * 
   * 那么最终请求的链接是 `https://趣助理.li/趣助理.version.json5`
   *
   */
...
export interface RawCommonProps {
   ...
   *
   * 比如 [Image &lt; \@View + View &gt;2 [text*='广告']](https://github.com/miaoqidong/subscription/blob/1ae87452d287b558f58f9c4e4448a3190e212ca1/src/apps/com.zidongdianji.ts#L26) 虽然符合快速查询的条件但是使用 `findAccessibilityNodeInfosByText("广告")` 并不能查询到节点
   *
   * 它是优点是快速, 因为遍历所有节点是一个耗时行为, 虽然多数情况下这种耗时较低
   *
   * 但是在某些软件比如 哔哩哔哩 的开屏广告在这种耗时下延迟可达 1-2s, 这也是导致 [miaoqidong/趣助理#60](https://github.com/miaoqidong/趣助理/issues/60) 的原因
   *
   ...
   *
   * 趣助理 借助 [onAccessibilityEvent](https://developer.android.com/reference/android/accessibilityservice/AccessibilityService#onAccessibilityEvent(android.view.accessibility.AccessibilityEvent)) 感知界面变化
   *
...
export interface RawRuleProps extends RawCommonProps {
  ...
  /**
   * 一个或者多个合法的 趣助理 选择器, 如果每个选择器都能匹配上节点, 那么点击最后一个选择器的目标节点
   */
  matches?: IArray<string>;

  /**
   * 一个或者多个合法的 趣助理 选择器, 如果存在一个选择器匹配上节点, 则停止匹配此规则
   */
 ...
 *
 * 示例2-点击目标节点的右上区域
 * - [快照-1](https://i.趣助理.li/i/14112390)
 * - [快照-2](https://i.趣助理.li/i/14319672)
 * - [图片-1](https://a.趣助理.li/0019.gif)
 * ```json5
 * {
 *  right: 'width*0.1352',
 *  top: 'width*0.0852',
